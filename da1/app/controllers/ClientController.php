    ...
    if (isset($_GET["type"]) && !empty($_GET["type"])) {
        switch ($_GET["type"]) {
            case "dh":
                ...
                break;
            case "dhct":
                $list_dhct = list_dh($_SESSION["khachhang"], $_GET["id"]);
            ...
            case "tt":
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST"){
                    $phantram_km = 0;
                    if(!empty($_POST["km"])){
                        if(check_km($_POST["km"]) != false){
                            $phantram_km = check_km($_POST["km"]);
                        }
                    }
                }
            ...
            case "add":
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    if (isset($_POST["add"]) && !empty($_POST["add"])) {
                        $list_gh_bt = list_gh_bt(implode(",", array_column($_SESSION["cart"], "id_bt")));
                        $idBill = addOrder($_SESSION["khachhang"], $_POST["sdt_dh"], $_POST["diachi_dh"], $_POST["ghichu_dh"], $_POST["giagoc_dh"], $_POST["giakm_dh"], $_POST["phantram_km"]);
