...
// ========================================TIM KIEM THEO TU KHOA==========================
    function timkiem($tukhoa,$type){
        ...
        if($type == 1){
            $sql = "SELECT * FROM san_pham WHERE ten_sp LIKE'%"  . $tukhoa . "%' GROUP BY gia_sp DESC";       
            return pdo_qr($sql);
        }

        if($type == 2){
            $sql = "SELECT * FROM san_pham WHERE ten_sp LIKE'%"  . $tukhoa . "%' GROUP BY gia_sp ASC";       
            return pdo_qr($sql);
        }
    }
...
// ============================================KM================================
function check_km($km){
    $sql = "SELECT * FROM khuyen_mai";
    $temp = pdo_qr($sql);

    foreach($temp as $mkm){
        if($mkm["ma_km"] == $km){
            return $mkm["phantram_km"];
        }
    }
    return false;
}
...
// ===========================================DAT HANG========================================
function addOrder($id_nd, $sdt_dh, $diachi_dh, $ghichu_dh, $giagoc_dh, $giakm_dh, $phantram_km) {
    $ngaydat_dh = date("Y-m-d");  // Định dạng ngày tháng năm để phù hợp với MySQL
    $sql = "INSERT INTO don_hang (id_nd, sdt_dh, diachi_dh, ghichu_dh, ngaydat_dh, giagoc_dh, giakm_dh, km_dh, trangthai_dh) VALUES ($id_nd, '$sdt_dh', '$diachi_dh', '$ghichu_dh', '$ngaydat_dh', $giagoc_dh, $giakm_dh, $phantram_km, 1);";
    pdo_exe($sql);
